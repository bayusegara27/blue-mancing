...
impl FishStats {
    ...
    fn summarize_fishing(&self) -> (HashMap<String, HashMap<String, HourlyStats>>, Vec<String>) {
        ...
        for entry in &self.fish_logs {
            ...
            if entry.caught {
                ...
                // Use "Unknown Fish" for catches where fish type detection failed
                let fish_type = entry
                    .fish_type
                    .clone()
    ...
    pub fn get_daily_table(&mut self, date: &str) -> String {
        ...
        let avg_xp_hour = if total_hours > 0.0 {
            total_xp as f64 / total_hours
        } else {
            0.0
        };

        let total_all = total_caught + total_missed;
        let catch_rate = if total_all > 0 {
            (total_caught as f64 / total_all as f64) * 100.0
        } else {
            0.0
        };
    ...
    pub fn get_overall_summary(&mut self) -> String {
        ...
        let overall_rate = if total_fish > 0 {
            (total_caught as f64 / total_fish as f64) * 100.0
        } else {
            0.0
        };
        ...
        let total_hours = self
            ...
            .map(|hours| hours.len())
            .sum::<usize>() as f64;
        let avg_fpm = if total_hours > 0.0 {
            total_caught as f64 / (total_hours * 60.0)
        } else {
            0.0
        };
...
impl StatsApi {
    ...
    pub fn set_resolution(&mut self, res: &str) {
        self.settings
            .insert("resolution".to_string(), res.to_string());
        self.save_settings();
    }
    ...
    pub fn set_show_debug_overlay(&mut self, value: bool) {
        self.settings
            .insert("show_debug_overlay".to_string(), value.to_string());
        self.save_settings();
    }
    ...
    pub fn set_overlay_always_on_top(&mut self, value: bool) {
        self.settings
            .insert("overlay_always_on_top".to_string(), value.to_string());
        self.save_settings();
    }
    ...
    pub fn set_show_overlay(&mut self, value: bool) {
        self.settings
            .insert("show_overlay".to_string(), value.to_string());
        self.save_settings();
    }
